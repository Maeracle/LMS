...
def login()->None:
    if "librarian" in session:
        ...
        if request.method == "POST":
            ...
            user:list = userlogin('Librarian',username=username,password=password)
            if len(user) > 0:
...
def register()->None:
    if 'librarian' in session:
        ...
        if request.method == "POST":
            ...
            valid = checkfields(name,email,username,password)
            if valid:
                ok:bool = addrecord('Librarian',l_name=name,l_email=email,username=username,password=password)
                if ok:
                    l_id = str(getLibrarianId(username)[0]['l_id'])
                    cart = addrecord('Cart',cart_id=l_id)
